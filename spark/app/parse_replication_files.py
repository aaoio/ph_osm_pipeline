...
# Exclude open changesets and changesets with centroid outside PH bounds
...
df = df\
    .withColumn('in_ph',
        in_ph(functions.array(df['centroid_lon'], df['centroid_lat'])))
...
repl_df = df.select(
    (df._closed_at.cast(types.TimestampType()).alias('closed_at')),
    (df._comments_count.cast(types.IntegerType())\
        .alias('comments_count')),
    (df._created_at.cast(types.TimestampType()).alias('created_at')),
    (df._id.cast(types.IntegerType()).alias('id')),
    (df._max_lat.cast(types.DoubleType()).alias('max_lat')),
    (df._max_lon.cast(types.DoubleType()).alias('max_lon')),
    (df._min_lat.cast(types.DoubleType()).alias('min_lat')),
    (df._min_lon.cast(types.DoubleType()).alias('min_lon')),
    'centroid_lat',
    'centroid_lon',
    (df._num_changes.cast(types.IntegerType()).alias('num_changes')),
    (df._uid.cast(types.IntegerType()).alias('uid')),
    (df._user.cast(types.StringType()).alias('username')),
    'region_relation_id',
    'province_relation_id',
    'city_municipality_relation_id'
)
